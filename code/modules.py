...
class DualNet50(NeuralNetwork):
  ...
  def build_graph(self, input):
    ...
    with tf.variable_scope(self.scope_name):
      ...
      conv2_lower = self.conv2d_relu(drop1_lower, filter_shape=[3, 3, 40, 40], scope_name="conv2")  # (b, 24, 24, 50)
      drop2_lower = self.dropout(conv2_lower, keep_prob=self.keep_prob, scope_name="drop2")

      conv3_lower = self.conv2d_relu(drop2_lower, filter_shape=[3, 3, 40, 50], scope_name="conv3")  # (b, 24, 24, 60)
      drop3_lower = self.dropout(conv3_lower, keep_prob=self.keep_prob, scope_name="drop3")
      ...
      deconv1 = self.deconv2d(up1, filter_shape=[2, 2], num_outputs=60, scope_name="deconv1")  # (b, 50, 50, 256)


      # ----- Fully connected layers ------
      concat1 = tf.concat([drop7_top, deconv1], axis=3)
      conv_1D_1 = self.conv2d(concat1, filter_shape=[1, 1, 120, 120], scope_name="conv1D_1")  # (b, 50, 50,, 128)
  ...
  def build_graph(self, input):
    ...
    with tf.variable_scope(self.scope_name, reuse=tf.AUTO_REUSE):
      ...
      for offset in offsets:
        ...
        # Initial convolutions
        conv1_top = self.conv2d_relu(green_box, filter_shape=[3, 3, 1, 60], scope_name="conv1_top")  # (b, 50, 50, 30) ALL ARE SAME SIZE
        conv2_top = self.conv2d_relu(conv1_top, filter_shape=[3, 3, 60, 60], scope_name="conv2_top")  # (b, 50, 50, 30)
        conv3_top = self.conv2d_relu(conv2_top, filter_shape=[3, 3, 60, 80], scope_name="conv3_top")  # (b, 50, 50, 40)
        conv4_top = self.conv2d_relu(conv3_top, filter_shape=[3, 3, 80, 80], scope_name="conv4_top")  # (b, 50, 50, 40)
        conv5_top = self.conv2d_relu(conv4_top, filter_shape=[3, 3, 80, 100], scope_name="conv5_top")  # (b, 50, 50, 50)
        conv6_top = self.conv2d_relu(conv5_top, filter_shape=[3, 3, 100, 100], scope_name="conv6_top")  # (b, 50, 50, 50)
        conv7_top = self.conv2d_relu(conv6_top, filter_shape=[3, 3, 100, 120], scope_name="conv7_top")  # (b, 50, 50, 60)
        ...
        conv1_lower = self.conv2d_relu(pool1_lower, filter_shape=[3, 3, 1, 80], scope_name="conv1")  # (b, 25, 25, 40) ALL SAME SIZE
        pool1_lower = self.maxpool2d(blue_box, scope_name='pool1_lower') # (b, 25, 25, 1)

        # Convolutions
        conv2_lower = self.conv2d_relu(conv1_lower, filter_shape=[3, 3, 80, 100], scope_name="conv2")  # (b, 25, 25, 50)
        conv3_lower = self.conv2d_relu(conv2_lower, filter_shape=[3, 3, 100, 120], scope_name="conv3")  # (b, 25, 25, 60)
        conv4_lower = self.conv2d_relu(conv3_lower, filter_shape=[3, 3, 120, 120], scope_name="conv4")  # (b, 25, 25, 60)
        conv5_lower = self.conv2d_relu(conv4_lower, filter_shape=[3, 3, 120, 120], scope_name="conv5")  # (b, 25, 25, 60)

        up1 = self.upsample(conv5_lower, scope_name="up1", factor=[2, 2])  # (b, 50, 50, 60)
        deconv1 = self.deconv2d(up1, filter_shape=[2, 2], num_outputs=120, scope_name="deconv1")  # (b, 50, 50, 60)
        deconv2 = self.deconv2d(deconv1, filter_shape=[2, 2], num_outputs=120, scope_name="deconv2")  # (b, 50, 50, 60)

        # ----- Fully connected layers ------
        concat1 = tf.concat([conv7_top, deconv1], axis=3)
        conv_1D_1 = self.conv2d(concat1, filter_shape=[1, 1, 240, 240], scope_name="conv1D_1")  # (b, 50, 50, 120)
        conv_1D_2 = self.conv2d(conv_1D_1, filter_shape=[1, 1, 240, 240], scope_name="conv1D_2")  # (b, 50, 50, 120)
  ...
  def build_graph(self, input):
    ...
    with tf.variable_scope(self.scope_name, reuse=tf.AUTO_REUSE):
      ...
      for offset in offsets:
        ...
        # Initial convolutions
        conv1_top = self.conv2d_relu(green_box, filter_shape=[3, 3, 1, 20], scope_name="conv1_top")  # (b, 50, 50, 30) ALL ARE SAME SIZE
        conv2_top = self.conv2d_relu(conv1_top, filter_shape=[3, 3, 20, 20], scope_name="conv2_top")  # (b, 50, 50, 30)
        conv3_top = self.conv2d_relu(conv2_top, filter_shape=[3, 3, 20, 30], scope_name="conv3_top")  # (b, 50, 50, 40)
        ...
        cropped_input = tf.slice(padded_input, [0, offset[0]-25, offset[1]-25, 0], [-1, 100, 100, -1]) # (b, 100, 100, 1)
        pool1_lower = self.maxpool2d(cropped_input, scope_name='pool1_lower') # (b, 50, 50, 1)
        conv1_lower = self.conv2d_relu(pool1_lower, filter_shape=[3, 3, 1, 20], scope_name="conv1")  # (b, 25, 25, 40) ALL SAME SIZE
        pool2_lower = self.maxpool2d(conv1_lower, scope_name='pool2_lower') # (b, 25, 25, 20)
        conv2_lower = self.conv2d_relu(pool2_lower, filter_shape=[3, 3, 20, 30], scope_name="conv2")  # (b, 25, 25, 50)
        reshape1 = tf.reshape(conv2_lower, shape=[-1, 25*25*30])  # (18750)
        fc1 = self.fc(reshape1, output_shape=1024, scope_name="fc1")        
        fc2 = self.fc(fc1, output_shape=256, scope_name="fc2")

        # Deconvolutions
        fc3 = self.fc(fc2, output_shape=1024, scope_name="fc3")        
        fc4 = self.fc(fc3, output_shape=25*25*30, scope_name="fc4")
        ...
        #reshape3 = tf.reshape(conv_1D_1, shape=[-1, 50*50*30])  # (75000)
        #fc5 = self.fc(reshape3, output_shape=1024, scope_name="fc5")
        #fc6 = self.fc(fc5, output_shape=50*50*1, scope_name="fc6") 
        #reshape4 = tf.reshape(fc6, shape=[-1, 50, 50, 1]) 
...
class DualNetFC2(NeuralNetwork):
  ...
  def build_graph(self, input):
    ...
    with tf.variable_scope(self.scope_name, reuse=tf.AUTO_REUSE):
      ...
      conv1_lower = self.conv2d_relu(pool1_lower, filter_shape=[3, 3, 1, 20], scope_name="conv1_lower")

      pool2_lower = self.maxpool2d(conv1_lower, scope_name='pool2_lower') # (b, 49, 49, 1)
      ...
      reshape1 = tf.reshape(pool3_lower, shape=[-1, 25*25*30])  # (18750)
      fc1 = self.fc(reshape1, output_shape=1024, scope_name="fc1_lower")        
      fc2 = self.fc(fc1, output_shape=256, scope_name="fc2_lower")

      # Fully connected layers
      fc3 = self.fc(fc2, output_shape=1024, scope_name="fc3_lower")        
      fc4 = self.fc(fc3, output_shape=25*25*30, scope_name="fc4_lower")
  ...
  def build_graph(self, input):
    ...
    with tf.variable_scope(self.scope_name, reuse=tf.AUTO_REUSE):
      ...
      for offset in offsets:
        ...
        conv5_top = self.conv2d_relu(reshape2_top, filter_shape=[3, 3, 20, 60], scope_name="conv5_top")  # (b, 50, 50, 50)
        ...
        up1 = self.upsample(reshape2, scope_name="up1", factor=[2, 2])  # (b, 50, 50, 30)
        deconv1 = self.deconv2d(up1, filter_shape=[2, 2], num_outputs=60, scope_name="deconv1")  # (b, 50, 50, 40)
